...
export async function reloadFunc() {
  try {
    ...
    console.error("Scene path:", this.params.scene);
    // Optionally revert to a working scene
    this.params.scene = "skydio_x2/scene_test.xml";
  }
}
...
export function setupGUI(parentContext) {
  ...
  parentContext.gui.add(parentContext.params, 'scene', {
    "Skydio": "skydio_x2/scene_test.xml", "Humanoid": "humanoid.xml", "Cassie": "agility_cassie/scene.xml",
...
export async function loadSceneFromURL(mujoco, filename, parent) {
    console.log("Starting to load scene:", filename);
    
    // Free the old simulation.
    if (parent.simulation != null) {
      console.log("Freeing old simulation");
...
export async function downloadExampleScenesFolder(mujoco) {
  let allFiles = [
    ...
    "skydio_x2/x2.xml",
    "skydio_x2/scene_x2.xml",
    "skydio_x2/scene_test.xml"
  ];
